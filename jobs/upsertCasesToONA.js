...
fn((state) => {
  ...
  const caseMappings = state.cases.map((c) => {
    ...
    const setViaLocation = (arr, searchVal, attributeToReturn) => {
      ...
      if (result) {
        //console.log("RESULT");
        //console.log(result);
        //console.log("attributeToReturn");
        //console.log(result[attributeToReturn]);
        return result[attributeToReturn];
      }
      return null;
    };
  ...
  const filteredCases = caseMappings.map((obj) => {
    const newObj = { ...obj };
    pii.forEach((key) => delete newObj[key]);
    return newObj;
  });

  return { ...state, caseMappings, filteredCases };
});
...
upsertMany(
  ...
  (state) => {
    console.log(
      "Data to be loaded to ONA",
      JSON.stringify(state.filteredCases, null, 2)
    );
    return state.caseMappings;
  },
  {
    writeSql: true,
    logValues: false,
